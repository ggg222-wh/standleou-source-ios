...
ImFont* logo; // I don't really want to fuck around with this shit placement fuck you

preview_model_t c_user_interface::m_stella { 1203, 3617 };
preview_model_t c_user_interface::m_ayanami_rei { 639, 1341 };

preview_base_t c_user_interface::m_preview_base { };
...
void c_user_interface::initialize( float scale )
{
    ...
    c_esp::m_fonts.pixel = GetIO( ).Fonts->AddFontFromMemoryTTF( smallest_pixel_ttf, sizeof smallest_pixel_ttf, 10 * scale, &mono_cfg, GetIO( ).Fonts->GetGlyphRangesCyrillic( ) );
    c_esp::m_fonts.verdana = GetIO( ).Fonts->AddFontFromMemoryTTF( verdana_ttf, sizeof verdana_ttf, 10 * scale, &main_cfg, GetIO( ).Fonts->GetGlyphRangesCyrillic( ) );
    c_esp::m_fonts.weapons = GetIO( ).Fonts->AddFontFromMemoryTTF( custom_weapon_ttf, sizeof custom_weapon_ttf, 14 * scale, &mono_cfg, GetIO( ).Fonts->GetGlyphRangesCyrillic( ) );
    c_esp::m_fonts.inferno_svg = GetIO( ).Fonts->AddFontFromMemoryTTF( inferno_svg_data, sizeof inferno_svg_data, 15 * scale, &mono_cfg, GetIO( ).Fonts->GetGlyphRangesCyrillic( ) );

    c_esp::m_nickname_fonts.push_back( { GetIO( ).Fonts->AddFontFromMemoryTTF( verdana_ttf, sizeof verdana_ttf, 10 * scale, &main_cfg, GetIO( ).Fonts->GetGlyphRangesCyrillic( ) ), 10 } );
    c_esp::m_nickname_fonts.push_back( { GetIO( ).Fonts->AddFontFromMemoryTTF( sf_pro_bold_ttf, sizeof sf_pro_bold_ttf, 12 * scale, &main_cfg, GetIO( ).Fonts->GetGlyphRangesCyrillic( ) ), 12 } );
    ...
    m_tabs = {
        { crypt_string( "ESP" ), 0 },
        { crypt_string( "Chams" ), 2 },
        { crypt_string( "World" ), 5 },
        { crypt_string( "Ragebot" ), 6 },
        { crypt_string( "Misc" ), 10 },
        { crypt_string( "Configs" ), 13 },
        { crypt_string( "Inventory" ), 15 } };

    m_subtabs = {
        // esp
        { crypt_string( "Enemies" ), 0 },
        { crypt_string( "World" ), 0 },

        // chams
        { crypt_string( "Enemies" ), 1 },
        { crypt_string( "Local" ), 1 },
        { crypt_string( "Weapon" ), 1 },

        // world
        { crypt_string( "World" ), 2 },

        // ragebot
        { crypt_string( "Main" ), 3 },
        { crypt_string( "Hitboxes" ), 3 },
        { crypt_string( "Anti-Aim" ), 3 },
        { crypt_string( "Exploits" ), 3 },

        // misc
        { crypt_string( "Misc" ), 4 },
        { crypt_string( "Spoof" ), 4 },
        { crypt_string( "Chat" ), 4 },

        // config
        { crypt_string( "Configs" ), 5 },
        { crypt_string( "Inventory" ), 5 },

        // inventory
        { crypt_string( "Inventory" ), 6 } };
}
    ...
    Begin( ay_obfuscate( "##TEKCT HAPUCAJI DMITRII UTKIN =)" ), 0, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoBringToFrontOnFocus );
    {
        ...
        { // left side of this fucking дерьмо ебанное
            ...
            BeginChild( ay_obfuscate( "##left_side" ), { left_side_width, wnd_rect.GetHeight( ) }, 0 );
            {
                ...
                if ( g_ctx->user_data.profile_picture.loaded )
                    render_user_info( g_ctx->user_data.username, g_ctx->user_data.premium, ( ImTextureID ) g_ctx->user_data.profile_picture.texture_id );
